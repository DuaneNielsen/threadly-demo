...
function to_webhook_payload_payments(tag, timestamp, record)
    local msg = record["log"] or record["message"] or ""
    local new_record = {
        alarm_name        = "ThreadlyPayments Log Error",
        severity          = "Danger",
        status            = "OPEN",
        host              = os.getenv("DEMO_HOST") or "threadly-host",
        agent             = "fluent-bit|ThreadlyPayments|Log Monitor",
        component_name    = "ThreadlyPayments",
        message           = msg,
        alert_external_id = "fluentbit:payments:log-error",
        metric_name       = "Log Events|ERROR:Rate",
        metric_value      = "1",
        caution_threshold = "5",
        danger_threshold  = "10",
